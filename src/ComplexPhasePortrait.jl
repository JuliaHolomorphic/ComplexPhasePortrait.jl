...
export portrait, PortraitType, PTproper, PTcgrid, PTstepphase, PTstepmod
...
* ctype - is the colormap type. The default "standard" is a basic HSL map with H∈[0,360], S = 1.0, and L = 0.5. Setting `ctype="nist"` gives the [NIST color scheme](http://dlmf.nist.gov/help/vrml/aboutcolor).
* pres - is the phase step resolution (number of phase jumps), default `pres=20`.
"""
function portrait(fval::Array{Complex{Float64},2}, mtype::PortraitType=PTproper;
    ...
    (n, m) = size(nphase)
    img = Image(Array(RGB{Float64}, n, m))
